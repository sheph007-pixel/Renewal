...
type SortKey = "plan" | "carrier" | "enrolled" | "monthly" | TierKey;

/** The plan's name as Employee Navigator spells it, year and all; the filed name on the shipped census. */
const planName = (p: { plan: string; enName?: string }) => p.enName || p.plan;
...
export default function Current({ data, overrides, g, rows, totals, invoiceHref }: Props) {
  ...
  const sorted = useMemo(() => {
    const value = (r: PlanRow): string | number => {
      if (sort.key === "plan") return planName(r.p).toLowerCase();
      if (sort.key === "carrier") return `${(r.p.tpa || "").toLowerCase()} ${planName(r.p).toLowerCase()}`;
