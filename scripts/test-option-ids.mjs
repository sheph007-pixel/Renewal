...
// 0. Nothing is numbered before a proposal arrives: the seed's menu quotes
//    are not proposals, so the payload carries no menu numbers.
const page0 = await (await fetch(`${base}/api/signin`, { method: "POST", headers: { ...json, cookie }, body: "{}" })).json();
assert.equal(page0.uhc.optionIds, undefined, "no menu numbering");
...
//     and a plan that comes back gets a fresh number; a surviving plan keeps
//     its number; a retired number is never handed out again.
...
assert.deepEqual(storedIds(lf3), ["UH7:P5000i10021B", "UH3:P3500i100ES21B", "UH8:Custom 7000"], "a plan that came back after its number was retired gets a fresh one");
...
assert.deepEqual(storedIds(lf5), ["UH3:P3500i100ES21B", "UH9:Custom 7000"], "a retired number is never handed out again");
...
assert.deepEqual((await storedFor(groupB))["UHC Level Funded"], ["UH1:P4000i8021B", "UH2:P4000i8021B EPO", "UH3:P5000i10021B"]);
...
assert.deepEqual(await storedFor(groupB), { "UHC Fully Insured": ["UH5:EZ2B Open Access HSA", "UH6:EZ2B Open Access EPO"], "UHC Level Funded": ["UH1:P4000i8021B", "UH2:P4000i8021B EPO", "UH3:P5000i10021B"] }, "numbers already given and not held twice are kept");
...
const signup = await fetch(`${base}/api/group/signup`, { method: "POST", headers: { ...json, cookie }, body: JSON.stringify({ code: mine.code, plans: ["UH3 · P3500i100ES21B"], note: "" }) });
...
const mixCarrier = await post(["UH3 · P3500i100ES21B", "GR1 · Gravie Copay 1500 PPO"]);
...
const mixFunding = await post(["UH3 · P3500i100ES21B", "UH4 · EZ2B Open Access HSA"]);
...
assert.equal((await post(["UH3 · P3500i100ES21B", "UH9 · Custom 7000"])).status, 200, "two level-funded UHC plans go through");
...
const page = await (await fetch(`${base}/api/signin`, { method: "POST", headers: { ...json, cookie }, body: "{}" })).json();
const table = comparisonTable({ group: page.group, proposals: page.proposals, plans: ["UH9", "GR1"], includeCurrent: false });
assert.deepEqual(
  table.rows.filter((r) => r.section === "2027 options").map((r) => r.name),
  ["UnitedHealthcare Option UH9\nCustom 7000", "Gravie Option GR1\nGravie Copay 1500 PPO"],
);
